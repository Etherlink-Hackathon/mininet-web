...
This is a web application for the **Etherlink Summer Camp** that enables **offline stablecoin payments** (USDT/USDC) without internet connectivity. The application interfaces with a FastPay authority network running on `mininet-wifi` to verify and process transactions through local TCP communication.
...
```
┌─────────────────────┐    ┌─────────────────────┐    ┌─────────────────────┐
│   Web Frontend      │    │   Backend API       │    │  FastPay Network    │
...
- Running mininet-wifi with FastPay authorities
